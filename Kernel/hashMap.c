...
//casi un calco de https://github.com/petewarden/c_hashmap

map_t newHashMap()
{
    hashmapMap* m = (hashmapMap*) malloc(sizeof(hashmapMap));
    m->data = (hashmapElement*) malloc(sizeof(hashmapElement));
    m->tableSize = INITIAL_SIZE;
    m->size = 0;
    return m;
}
...
{
    unsigned long key = crc32((unsigned char*)(keystring), strleng(keystring));
...
int hashmapHash(map_t in, char* key)
{
    ...
    hashmapMap* m = (hashmapMap*) in;
    if(m->size >= (m->tableSize/2)) return MAP_FULL;

    curr = hashmapHashInt(m,key);

    for(i=0; i<MAX_CHAIN_LENGTH; i++)
    {
        if(m->data[curr].inUse==0)
            return curr;
        if(m->data[curr].inUse==1 && (strcmp(m->data[curr].key, key)==0))
            return curr;
...
int hashmapRehash(map_t in)
{
    ...
    hashmapElement* temp = (hashmapElement *)
            malloc(2 * m->tableSize * sizeof(hashmapElement));
...
/* Return the hashmapLength of the hashmap */
